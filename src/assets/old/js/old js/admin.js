...
$(document).on("click", ".sidemenu-collapse", function(e) {
    ...
    $body.toggleClass("submenu-closed");

    var sidebar_option = $body.hasClass("side-closed") ? "side-closed submenu-closed" : "";
    //You may not even need the sidebar-user-panel display logic. The CSS should handle the visibility based on the classes.
    //$(".sidebar-user-panel").css({ display: $body.hasClass("side-closed") ? "none" : "block" }); 
...
document.addEventListener('DOMContentLoaded', function() {
    setActiveLink('/home'); // Call the function with the desired path
});
...
function setActiveLink(targetPath) {
    ...
    links.forEach(link => {
        ...
        if (linkPath === targetPath) { //Exact match
            link.classList.add('active');

            let parentLi = link.closest('li');
            while (parentLi) {
                parentLi.classList.add('active');
                //Avoid applying 'active' to the top-level menu item
                parentLi = parentLi.parentElement.closest('li.menu-toggle');
            }
        } else {
            link.classList.remove('active'); // Remove active from other links

            // Remove "active" from parent <li> elements of other links
            let parentLi = link.closest('li');
            while (parentLi) {
    ...
    // Check if the user panel exists before attempting to set the active class
    if (userPanel) {
